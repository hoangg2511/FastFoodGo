...
class ChiTietMonAnViewModel extends ChangeNotifier {
  ...
  Future<Map<String, dynamic>?> fetchChiTietMonAn(String monAnId) async {
    try {
      ...
      // 1. Cập nhật monAnData và basePrice ngay lập tức
      monAnData = monAn;
      ...
      // 2. CHỈ fetch options nếu danh sách loại options KHÔNG null
      if (loaiOptionList != null && loaiOptionList.isNotEmpty) {
        ...
        // Bắt đầu quá trình fetch từng nhóm option
      ...
      // 3. Khối code này LUÔN chạy (để cập nhật state cho UI)
      _chiTietMonAn = {
        'monAn': monAn,
        'optionMap': optionMap,
      };
      optionData = optionMap; // Cập nhật state optionData cho UI
